...
(function () {
    ...
    angular.module('BlurAdmin.pages.customer.leaveMessage')
        .controller('LeaveMessagePageCtrl', LeaveMessagePageCtrl);

    /** @ngInject */
    function LeaveMessagePageCtrl($scope,$uibModal, baProgressModal, $filter,appBase) {
		...
		$scope.open = function (page, size, id, name) {
			$uibModal.open({
				animation: true,
				templateUrl: page,
				size: size,
				controller: 'LeaveMessageModalCtrl',
				resolve: {
					modelId: id,
					name : name,
				}
		...
		$scope.showButton = function(index,status){
            if(status == '1'){
            	return false;
            }
            return true;
